...
// Out-of-proc COM server hosting the modern Windows 11 ExplorerCommand for 7-Zip.
//
// Build (MSVC x64):
//   cl /EHsc /W4 /permissive- /std:c++17 7Zip.ShellExtensionHost.cpp ^
//      Ole32.lib Shell32.lib Shlwapi.lib /link /SUBSYSTEM:WINDOWS
//
// MSIX manifest should register this EXE via <com:ExeServer> with the same CLSID
// and add a single modern context menu verb under
// <desktop4:Extension Category="windows.fileExplorerContextMenus"> pointing to that CLSID.
//
// Behavior:
...
//  - Add/Email entries available broadly (like classic).
...
static void ShellRun(const std::wstring& exe, const std::wstring& args) {
...
static std::wstring DefaultArchiveName(const std::vector<std::wstring>& paths, const wchar_t* ext) {
    ...
    // Multi-selection → use parent directory name if all share the same parent
...
struct ExplorerCommandRoot : IExplorerCommand, IEnumExplorerCommand {
    ...
    IFACEMETHODIMP GetIcon(IShellItemArray*,LPWSTR*pp)override{*pp=nullptr;return E_NOTIMPL;}
...
// message loop (quits when no objects and no locks)
static void Pump() {
    MSG msg;
